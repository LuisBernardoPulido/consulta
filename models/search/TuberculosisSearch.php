 ...
 */
class TuberculosisSearch extends Tuberculosis
{
    public $fecha_desde;
    public $fecha_hasta;
    /**
    ...
    public function rules()
    {
        return [
            ...
            [['p03_fpruebaAnt', 'p03_fproxPrueba', 'p03_espTipo', 'p03_espMotivo', 'p03_finyeccion', 'p03_flectura', 'p03_tipoAux', 'p03_fecha', 'p03_constHatoFecha', 'p03_vigencia', 'p03_fecAlta', 'p03_fecMod', 'p02_id', 'fecha_desde', 'fecha_hasta'], 'safe'],
        ];
    }
    ...
    public function search($params)
    {
        ...
        if(User::isUserSupervisor(Yii::$app->user->getId()) || User::isUserCoordinador(Yii::$app->user->getId())){
            $query = Tuberculosis::find()->where('p03_activo=:activo', [':activo'=>1])->orderBy('p03_fecAlta DESC');
        }else if(User::isUserAdmin(Yii::$app->user->getId())){
            $query = Tuberculosis::find()->where('p03_activo=:activo', [':activo'=>1])->orderBy('p03_fecAlta DESC');
        }else{
            $grupos = Grupos::getUsuarios();
            $query = Tuberculosis::find()->where('p03_usuAlta=:usuario || c05_id=(SELECT c05_id FROM c05_mvz WHERE user_id=:usuario)',[':usuario'=>Yii::$app->user->getId()]);
