function validarInicio(){
    var ini =document.getElementById('folioscoordinador-r17_rangoinicio').value;
    ...
    if(ini!=''){
        ...
        $.ajax({
            type: 'GET',
            url: 'index.php?r=folios-coordinador/validar_fol_ini',
            data: val,
            dataType: "json",
            success: function (res) {
                if(res==1){
                    ...
                    document.getElementById('folioscoordinador-r17_rangoinicio').value='';
                }else if(res==2){
                    swal({
                        type: 'error',
                        title: 'No se puede asignar el folio ' + ini + '.',
...
function validarFin(){
    var ini =document.getElementById('folioscoordinador-r17_rangoinicio').value;
    var fin =document.getElementById('folioscoordinador-r17_rangofin').value;
    var tipo =document.getElementById('tipo_dict').value;
    var id = document.getElementById('id_registro').value;
    if(ini!='' && fin!='' && validarOrden(ini, fin)){
        ...
        $.ajax({
            type: 'GET',
            url: 'index.php?r=folios-coordinador/validar_fol_fin',
            data: val,
            dataType: "json",
            success: function (res) {
                if(res==1){
                    ...
                    document.getElementById('folioscoordinador-r17_rangoinicio').value='';
                    document.getElementById('folioscoordinador-r17_rangofin').value='';
                }else if(res==2){
                    swal({
                        type: 'error',
                        title: 'No se puede asignar el rango de folios del ' + ini + ' al '+ fin +'.',
...
function foliosPendientes() {
    var med =document.getElementById('folioscoordinador-c05_id').value;
    if(med!=''){
        val = {"med": med};
        $.ajax({
            type: 'GET',
            url: 'index.php?r=folios-coordinador/folios_pendientes',
            data: val,
            dataType: "json",
            success: function (res) {
                if(res){
